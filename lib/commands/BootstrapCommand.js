  ...
  _createClass(BootstrapCommand, [{
    ...
    value: function symlinkPackages(callback) {
      ...
      this.filteredPackages.forEach(function (filteredPackage) {
        var packageLocation = _path2.default.join(_this6.repository.packagesLocation, filteredPackage.name);
        ...
        .filter(function (dependency) {
          var match = (0, _lodash2.default)(_this6.packages, { name: dependency });
          return match && filteredPackage.hasMatchingDependency(match);
        }).forEach(function (dependency) {
          // get path to dependency and its package.json
          var dependencyLocation = _path2.default.join(_this6.repository.packagesLocation, dependency);
          ...
          if (!_FileSystemUtilities2.default.existsSync(dependencyPackageJsonLocation)) {
            _this6.logger.error("Unable to find package.json for " + dependency + " dependency. Skipping...");
          } else {
            (function () {
              var installedDepLocation = _path2.default.join(filteredPackage.nodeModulesLocation, dependency);
              // check if dependency is already installed
              if (_FileSystemUtilities2.default.existsSync(installedDepLocation)) {
                var isDepSymlink = _FileSystemUtilities2.default.isSymlink(installedDepLocation);
                ...
                if (isDepSymlink !== dependencyLocation) {
                  ...
                  packageActions.push(function (cb) {
                    return _FileSystemUtilities2.default.rimraf(installedDepLocation, cb);
                  });
                }
              }
              // ensure node_modules folder
              packageActions.push(function (cb) {
                return _FileSystemUtilities2.default.mkdirp(_path2.default.join(packageLocation, "node_modules"), cb);
              });
              // create package symlink
              var dependencyLinkLocation = _path2.default.join(packageLocation, "node_modules", dependency);
              packageActions.push(function (cb) {
                return _FileSystemUtilities2.default.symlink(dependencyLocation, dependencyLinkLocation, "dir", cb);
              });
              var dependencyPackageJson = require(dependencyPackageJsonLocation);
              if (dependencyPackageJson.bin) {
                (function () {
                  var destFolder = _path2.default.join(_this6.repository.packagesLocation, filteredPackage.name, "node_modules");
                  packageActions.push(function (cb) {
